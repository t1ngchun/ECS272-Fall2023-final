...
var Prop = _createClass(function Prop2(name, _ref) {
  var _ref$default = _ref["default"], defaultVal = _ref$default === void 0 ? null : _ref$default, _ref$triggerUpdate = _ref.triggerUpdate, triggerUpdate = _ref$triggerUpdate === void 0 ? true : _ref$triggerUpdate, _ref$onChange = _ref.onChange, onChange3 = _ref$onChange === void 0 ? function(newVal, state) {
  } : _ref$onChange;
  _classCallCheck(this, Prop2);
  this.name = name;
  this.defaultVal = defaultVal;
  this.triggerUpdate = triggerUpdate;
  this.onChange = onChange3;
});
  ...
  return function() {
    ...
    props.forEach(function(prop) {
      ...
      function getSetProp(_ref3) {
        var prop2 = _ref3.name, _ref3$triggerUpdate = _ref3.triggerUpdate, redigest = _ref3$triggerUpdate === void 0 ? false : _ref3$triggerUpdate, _ref3$onChange = _ref3.onChange, onChange3 = _ref3$onChange === void 0 ? function(newVal, state2) {
        ...
        return function(_) {
          ...
          state[prop2] = val;
          onChange3.call(comp, val, state, curVal);
...
// node_modules/svg-text-fit/dist/svg-text-fit.mjs
...
var ellipsis = index({
  ...
  init: function init2(el, state) {
    state.el = el;
  },
  update: function update2(state) {
    var origTxt = state.el.textContent;
    var nChars = Math.round(origTxt.length * state.maxWidth / state.el.getBBox().width * 1.2);
...
// node_modules/svg-utils/dist/svg-utils.mjs
var moveToFront = index({
  init: function init3(el) {
...
var gradient = index({
  ...
  init: function init4(el, state) {
    state.id = "areaGradient".concat(Math.round(Math.random() * 1e4));
    state.gradient = select_default(el).append("linearGradient");
  },
  update: function update3(state) {
    ...
    var colorStops = state.gradient.selectAll("stop").data(range(0, 100.01, 20));
...
var dropshadow = index({
  ...
  props: {
    id: {
      "default": "areaGradient".concat(Math.round(Math.random() * 1e4))
    }
  },
  init: function init5(el, state) {
    state.filter = select_default(el).append("defs").append("filter").attr("height", "130%");
...
var throbber = index({
  ...
  init: function init6(el, state) {
    el = select_default(el);
...
var image = index({
  ...
  init: function init7(el, state) {
    state.img = select_default(el).append("image");
  },
...
var zoomOverviewArea = index({
  props: {
    ...
    yDomain: {
      onChange: function onChange2(yDomain, state) {
  ...
  init: function init8(el, state, _ref) {
    var width = _ref.width, height = _ref.height, _ref$margin = _ref.margin, margin = _ref$margin === void 0 ? {
      top: 2,
      right: 2,
      bottom: 2,
      left: 2
    ...
    state.svg = select_default(el).append("svg").attr("width", width).attr("height", height);
...
// node_modules/d3-color-legend/dist/d3-color-legend.mjs
var ContinuousLegend = index({
  ...
  init: function init9(el, state) {
    state.gradient = gradient()(el);
    state.el = select_default(el);
...
var legend = index({
  ...
  init: function init11(el, state) {
    state.legend = select_default(el).append("g").attr("class", "legend");
  },
  update: function update10(state) {
    if (!state.scale)
      return;
    var isOrdinal = !state.scale.hasOwnProperty("interpolate") && !state.scale.hasOwnProperty("interpolator");
    state.legend.html("");
